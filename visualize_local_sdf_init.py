...
def main() -> None:
    ...
    scalings = data["scalings"] if "scalings" in data else None
    ...
    if not args.hide_normals and positions_normals.shape[0] > 0:
        normal_lines = build_normal_lines(
            positions_normals,
            normals_normals,
            args.normal_scale,
        )
    ...
    if not args.no_frame:
        extent = positions.max(axis=0) - positions.min(axis=0) if positions.size else np.ones(3, dtype=np.float32)
